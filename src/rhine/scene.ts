...
export class ArchiveScene {
  ...
  private drawLabel(index: number) {
    ...
    c.font = "bold 81px 'Alibaba PuHuiTi', sans-serif";
    // Each printed card carries its own template, matching the templates page.
    c.fillText(records[index]?.en ?? "RESUME EDITOR", 22, 116);
    c.font = "32px 'Alibaba PuHuiTi', sans-serif";
    c.fillStyle = "#878476";
    c.fillText("RESUME EDITOR TEMPLATE", 25, 174);
    c.fillStyle = "#171713";
    c.font = "bold 130px 'Alibaba PuHuiTi', sans-serif";
    c.fillText("NO." + String(index + 1).padStart(3, "0"), 22, 360);
    c.fillRect(782, 32, 221, 39);
    c.fillStyle = "#eee9de";
    c.font = "24px 'Alibaba PuHuiTi', sans-serif";
    c.fillText("R L / I S", 809, 61);
    c.fillStyle = "#171713";
    c.font = "bold 64px 'Alibaba PuHuiTi', sans-serif";
  ...
  private bindPointer() {
    ...
    window.addEventListener("blur", () => this.cancelPointer(), { signal });
    document.addEventListener("visibilitychange", () => {
      if (document.hidden) this.cancelPointer();
    }, { signal });
    window.addEventListener("resize", () => this.cancelPointer(), { signal });
    // After multi-touch cancels capture, a finger can finish outside the canvas.
    window.addEventListener("pointerup", (e) => pointers.delete(e.pointerId), { signal });
    window.addEventListener("pointercancel", (e) =>
      pointers.delete(e.pointerId), { signal });
  }
  update(
    time: number,
  ...
  /**
   * Mounted inside a host app, the scene must release its GL context and
   * global listeners when the portal route unmounts.
   */
  dispose() {
    this.teardown.abort();
